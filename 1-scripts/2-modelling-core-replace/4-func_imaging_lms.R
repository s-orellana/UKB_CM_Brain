...
mdd_scatt<- function(df, var, script_ID = NULL){
  
  #df: expects as input the output from the LM function. 
  
  #---------------------------A. Input MDD contrasts
  ...
  df<-  df %>% inner_join(., MDD_contrast, by = "BRAIN_ROI")
  

  #---------------------------B. Plot 
  df %>% filter(variable == var  & layer == "ctx") %>% 
    ggplot(., aes(x= t.statistic, y= mdd_laterlized_tstat) )+
    ...
    theme(text=element_text(size=30,family="sans"),
          ...
          legend.position = "right", aspect.ratio=5/4.5,
          legend.title = element_blank()) +
    xlab(expr(paste(!! var, " ", t))) +
    ylab(expr(paste("MDD case-control difference", " ",  t))) -> pplot
  
  #---------------------------D. Save
  plt_dir<- paste0(scriptdir, "3-plots/P4/", script_ID, "/",
                   "MDD_correlation_", var, ".png" )
  
  ggsave(plt_dir, pplot,  width=11, height=8)
